...
from flow.ledger.credits import consume_ai_credits, get_user_credit_balance


@frappe.whitelist()
def get_user_credit_status() -> dict[str, Any]:
	"""Return current user's AI credit balance and active status."""
	user = frappe.session.user
	balance = get_user_credit_balance(user)
	return {"user": user, "balance": balance, "status": "active"}
...
def start_run(
	...
	context_docname: str | None = None,
) -> dict[str, Any] | Response:
	"""Start a new turn. Creates a session if none is given. `attachments` are uploaded File
	names whose text is injected into this turn. Injects real-time active CRM screen context. With `stream=True`, returns SSE."""
	...
	out = convo.chat(
		...
		attachments=files,
		stream=stream,
		reference_doctype=context_doctype,
		reference_name=context_docname,
	)
