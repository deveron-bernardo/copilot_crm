...
class FlowSession(Document):
	...
	def chat(
		...
		*,
		...
		run_input = self._build_prompt_messages(reference_doctype=reference_doctype, reference_name=reference_name)
	...
	def resume(self, answers: dict[str, Any], *, stream: bool = False) -> FlowRun | Generator[Event]:
		...
		messages = self._build_prompt_messages(
			reference_doctype=getattr(run, "reference_doctype", None),
			reference_name=getattr(run, "reference_name", None),
		)
		if not messages:
	...
	def _build_prompt_messages(
		...
		- Active record context: rich summary of the CRM record open on screen.
		...
		"""
		from flow.assistant.context import build_active_record_summary
		...
		# Active CRM Record Context
		active_summary = build_active_record_summary(reference_doctype, reference_name)
		if active_summary:
			if messages and messages[0]["role"] == "system":
				messages[0]["content"] = f"{messages[0]['content']}\n\n{active_summary}"
			else:
				messages.insert(0, {"role": "system", "content": active_summary})
