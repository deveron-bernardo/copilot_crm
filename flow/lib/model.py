...
def resolve_provider_credentials(model_id: str) -> dict[str, Any]:
	"""Look up central Flow Provider credentials for a model's provider, with fallback to .env."""
	try:
		import litellm
	...
	if provider and frappe.db.exists("Flow Provider", provider):
	...
	# Fallback to .env / os.environ / site_config.json
	from flow.lib.env_config import get_ai_env_config

	env_conf = get_ai_env_config()
	if env_conf.get("api_key"):
		return {
			"api_key": env_conf.get("api_key"),
