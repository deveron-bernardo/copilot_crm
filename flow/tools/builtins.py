...
def read(
	...
	CRITICAL FRAPPE CRM NAMING CONVENTION:
	In Frappe, `name` (and `id`) is ALWAYS the unique document code (e.g. "CRM-LEAD-2026-00012", "CRM-DEAL-2026-00001"), NEVER the person's or deal's human name!
	...
	fetch_fields = list(fields) if fields else ["name"]
	if "name" not in fetch_fields:
		fetch_fields.insert(0, "name")

	# Automatically enrich with essential human-readable fields so the LLM never confuses ID with Name
	if meta:
		if meta.title_field and meta.title_field not in fetch_fields and meta.has_field(meta.title_field):
			fetch_fields.append(meta.title_field)

		if doctype == "CRM Lead":
			for key in ("lead_name", "first_name", "last_name", "organization", "email", "mobile_no", "status", "lead_owner"):
				if meta.has_field(key) and key not in fetch_fields:
					fetch_fields.append(key)
		elif doctype == "CRM Deal":
			for key in ("title", "organization", "status", "deal_owner", "annual_revenue", "expected_closure_date"):
				if meta.has_field(key) and key not in fetch_fields:
					fetch_fields.append(key)
		elif doctype == "CRM Organization":
			for key in ("organization_name", "territory", "industry", "website"):
				if meta.has_field(key) and key not in fetch_fields:
					fetch_fields.append(key)
		elif doctype == "CRM Task":
			for key in ("title", "status", "priority", "due_date", "assigned_to"):
				if meta.has_field(key) and key not in fetch_fields:
					fetch_fields.append(key)
		elif doctype == "Contact":
			for key in ("first_name", "last_name", "email_id", "phone", "mobile_no", "company_name"):
				if meta.has_field(key) and key not in fetch_fields:
					fetch_fields.append(key)
	...
	for r in records:
		if "name" in r:
			...
			r["codigo_id"] = r["name"]
		if doctype == "CRM Lead":
			full_name = r.get("lead_name") or f"{r.get('first_name') or ''} {r.get('last_name') or ''}".strip()
			if full_name:
				r["full_name"] = full_name
				r["lead_name"] = full_name
				r["nome_do_lead"] = full_name
			if r.get("organization"):
				r["empresa"] = r["organization"]
		elif doctype == "CRM Deal":
			if r.get("title"):
				r["deal_title"] = r["title"]
		elif doctype == "Contact":
