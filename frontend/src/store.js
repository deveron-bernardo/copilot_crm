...
const activeContext = ref(detectActiveCRMContext());
...
async function loadInitial() {
	try {
		...
		const assistant = agents.value.find((x) => x.name === "Flow");
		selectedAgent.value = assistant ? assistant.name : agents.value[0]?.name ?? null;
		...
		console.error("[Copilot CRM] loadInitial error:", err);
		// `loaded` stays false, keeping the composer disabled on "Loading…".
		if (typeof window !== "undefined" && typeof window.frappe?.show_alert === "function") {
...
async function refreshHistory() {
	try {
		...
		console.warn("[Copilot CRM] refreshHistory warning:", e);
...
async function send(text) {
	...
	const ctx = detectActiveCRMContext();
		...
		await startRun(
			{
				...
				...(ctx?.doctype && { context_doctype: ctx.doctype }),
				...(ctx?.docname && { context_docname: ctx.docname }),
			},
