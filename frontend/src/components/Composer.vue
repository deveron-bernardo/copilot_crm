...
const DEFAULT_SUPPORTED_FILE_TYPES = [
	"pdf", "xlsx", "docx", "html", "htm", "txt", "text", "md", "markdown",
	"csv", "tsv", "log", "rst", "json", "yaml", "yml", "png", "jpg", "jpeg",
	"webp", "bmp", "tiff", "tif", "gif",
];

// Backend (flow/boot.py) is the source of truth when bootinfo exists; fall back gracefully in SPAs like Frappe CRM.
const ACCEPT = computed(() => {
	const types =
		(typeof window !== "undefined" && window.frappe?.boot?.flow_supported_file_types) ||
...
const placeholder = computed(() => {
	...
	const label = agentLabel(selectedAgent.value) || "Flow";
	return __("Ask {0}…", [label]);
});
...
<template>
	<div
		class="flow-composer absolute inset-x-5 bottom-3.5 z-10 mx-auto flex max-w-3xl flex-col gap-1.5 rounded-xl border bg-surface-white px-2.5 py-2 shadow-sm transition-[border-color,background-color] focus-within:border-outline-gray-3"
		:class="dragging ? 'border-outline-gray-4 bg-surface-gray-1' : 'border-outline-gray-2'"
		@dragover="onDragOver"
		@dragleave="onDragLeave"
		@drop="onDrop"
	>
		<div v-if="activeContext" class="flex items-center gap-1.5 px-2 py-0.5 text-xs text-ink-gray-7 bg-surface-gray-2 rounded-md w-fit border border-outline-gray-2">
			<FeatherIcon name="target" class="h-3 w-3 text-ink-blue-3" />
			<span class="font-medium text-ink-gray-9">{{ activeContext.label }}</span>
			<button
				type="button"
				class="ml-1 text-xs font-bold leading-none text-ink-gray-4 hover:text-ink-gray-7"
				:title="__('Limpar contexto ativo')"
